...
export function NoteCard({ note }) {
    return (
        <div key={note.id} className="bg-[#111114] border border-white/[0.05] hover:border-white/10 rounded-xl p-5 flex flex-col gap-3 transition-all duration-200 cursor-pointer group hover:bg-[#151519] shadow-sm">
            ...
            <div className="flex justify-between items-center mt-auto pt-4 border-t border-white/[0.04]">
                ...
                <span className="text-slate-500 text-[11px] font-medium shrink-0">{note.createdAt}</span>
            </div>
        </div>
    );
}
